#!/usr/bin/env python3
"""
Script to clean existing duplicate images and properly import all images from imagesForLovMeNow directory
"""
...
def extract_identifier(directory_name):
    """Extract UPC or SKU from directory name"""
    # Remove SKU- or SKU_ prefix if present
    if directory_name.startswith('SKU-') or directory_name.startswith('SKU_'):
        return directory_name.replace('SKU-', '').replace('SKU_', '').zfill(4)
    
    # If it's all digits, it's likely a UPC
...
def find_matching_product_variant(identifier):
    ...
    # For SKU directories, try to match with wholesale_id patterns
    if not product and identifier.isdigit():
        # Try different wholesale_id patterns
        patterns_to_try = [
        ...
        for pattern in patterns_to_try:
            product = Product.query.filter_by(wholesale_id=pattern).first()
            if product:
                break
...
def clean_and_import_images():
    """Main function to clean existing images and import new ones"""
    app = create_flask_app()
    
    with app.app_context():
        print("🧹 Starting image cleanup and import process...")
        
        # Step 1: Clean existing images
        print("\n=== STEP 1: CLEANING EXISTING IMAGES ===")
        existing_images = ProductImage.query.all()
        print(f"Found {len(existing_images)} existing images to remove")
        ...
        for directory_name, images in images_by_directory.items():
            ...
            # Extract identifier (UPC or SKU)
            identifier = extract_identifier(directory_name)
            print(f"  Looking for product with identifier: {identifier}")
